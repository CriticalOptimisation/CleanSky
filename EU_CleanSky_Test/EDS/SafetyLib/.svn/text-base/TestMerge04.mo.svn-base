...
model TestMerge04 "Split high source to two loads."
  ...
  EU_CleanSky.EDS.SafetyLib.Link link(useOpen=false, useLeak=false) annotation (
     Placement(transformation(
        ...
        rotation=-90,
        origin={-10,-10})));
  EU_CleanSky.EDS.SafetyLib.Sources.LoPotential sourceP annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={-90,40})));
  EU_CleanSky.EDS.SafetyLib.Sources.LoPotential sink(gender=EU_CleanSky.EDS.SafetyLib.Gender.female)
  ...
  EU_CleanSky.EDS.SafetyLib.Sources.HiPotential sink1 annotation (Placement(
        transformation(
        extent={{10,10},{-10,-10}},
        rotation=-90,
        origin={-10,-50})));
...
equation
  connect(sourceP.m, load1.minusF) annotation (Line(
      points={{-80,40},{-76,40},{-76,40},{-60,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(load1.plusM, merge1.f) annotation (Line(
      points={{-40,40},{-20,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(merge1.m1, load2.plusF) annotation (Line(
      points={{0,40},{20,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(load2.minusM, sink.f) annotation (Line(
      points={{40,40},{60,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(merge1.m2, link.n) annotation (Line(
      points={{-10,32},{-10,16},{-10,0},{-10,0}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(link.p, sink1.f) annotation (Line(
