...
model TestMerge51 "Load in series - middle"
  import EU_CleanSky.EDS.SafetyLib;

  EU_CleanSky.EDS.SafetyLib.Merge merge1
    annotation (Placement(transformation(extent={{-20,50},{0,30}})));
  EU_CleanSky.EDS.SafetyLib.PolarisedLoad link3(
    ...
    final useReverseInterface=true) annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={-10,-10})));
  EU_CleanSky.EDS.SafetyLib.Cap source2(gender=EU_CleanSky.EDS.SafetyLib.Gender.female)
    annotation (Placement(transformation(extent={{50,30},{70,50}})));
  EU_CleanSky.EDS.SafetyLib.Link link2(useOpen=false, useLeak=false)
    annotation (Placement(transformation(extent={{40,30},{20,50}})));
  EU_CleanSky.EDS.SafetyLib.Sources.LoPotential source1(gender=EU_CleanSky.EDS.SafetyLib.Gender.female)
  ...
  EU_CleanSky.EDS.SafetyLib.Sources.HiPotential sink(gender=EU_CleanSky.EDS.SafetyLib.Gender.male)
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-90,40})));
  EU_CleanSky.EDS.SafetyLib.PolarisedLoad link1(
    semantics=EU_CleanSky.EDS.SafetyLib.Semantics.electrical,
    useLeak=false,
    useOpen=false,
...
equation
  connect(merge1.m2, link3.plusF) annotation (Line(
      points={{-10,32},{-10,16},{-10,0},{-10,0}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(sink.m, link1.plusF) annotation (Line(
      points={{-80,40},{-60,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(link1.minusM, merge1.f) annotation (Line(
      points={{-40,40},{-20,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(merge1.m1, link2.n) annotation (Line(
      ...
      color={255,0,255},
      smooth=Smooth.None));
  connect(link2.p, source2.n) annotation (Line(
      points={{40,40},{60,40}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(link3.minusM, source1.f) annotation (Line(
