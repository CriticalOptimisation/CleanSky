...
model ReverseControlledPower
  "A controlled power source emitting high and low potentials when boolean input is true, reversed."
...
<p>The law of probability attached to the &QUOT;loss of power&QUOT; failure mode of the Reverse Controlled Power model. It can be replaced by any law under Internal/ProbabilityDistribution/Continuous.</p>
  ...
  Interfaces.LogicFlowM n annotation (Placement(transformation(extent={{78,40},{
            82,44}}), iconTransformation(extent={{74,60},{86,72}})));
  Interfaces.LogicFlowF p annotation (Placement(transformation(extent={{-82,40},
  ...
  Modelica.Blocks.Interfaces.BooleanInput u annotation (Placement(
        transformation(extent={{-108,-28},{-92,-12}}),
            ...
            {-92,8}})));
protected
  Sources.ControlledHiPotential p1(gender=EU_CleanSky.EDS.SafetyLib.Gender.male)
    annotation (Placement(transformation(extent={{30,10},{50,30}})));
  Sources.ControlledLoPotential n1(gender=EU_CleanSky.EDS.SafetyLib.Gender.female)
    annotation (Placement(transformation(extent={{-30,10},{-50,30}})));
...
equation
  ...
  connect(loss.y, not1.u) annotation (Line(
      ...
      color={255,0,255},
      smooth=Smooth.None));
  connect(n1.f, p) annotation (Line(
      points={{-50,20},{-80,20},{-80,42}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(p1.m, n) annotation (Line(
      points={{50,20},{80,20},{80,42}},
      color={255,0,255},
      smooth=Smooth.None));
